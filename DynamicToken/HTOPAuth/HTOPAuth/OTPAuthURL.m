...
+ (NSString *)encodeBase32:(NSData *)data {
    ...
    [coder addDecodeSynonyms:kBase32Synonyms];
    [coder ignoreCharacters:kBase32Sep];
    return [coder encode:data error:nil];
}
...
- (id)initWithOTPGenerator:(OTPGenerator *)generator
    ...
    if ((self = [super init])) {
        if (!generator || !name) {
            NSLog(@"Bad Args Generator:%@ Name:%@", generator, name);
            [self release];
            self = nil;
        } else {
...
- (BOOL)saveToKeychain {
    ...
    if ([self isInKeychain]) {
        ...
        NSLog(@"SecItemUpdate(%@, %@) = %d", query, attributes, (int)status);
    } else {
        ...
        NSLog(@"SecItemAdd(%@, %@) = %d", attributes, ref, (int)status);
...
- (BOOL)removeFromKeychain {
    ...
    NSLog(@"SecItemDelete(%@) = %d", query, (int)status);
